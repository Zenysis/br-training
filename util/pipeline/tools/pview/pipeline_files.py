...
def find_search(file_filters=None, restrict_recency=True):
    ...
    if file_filters:
        ...
        for file_filter in file_filters:
            args.append('-iregex ".*%s.*"' % file_filter)
...
def run_search(command, max_files=-1):
    ...
    # NOTE(stephen): Prefer iterating over `process.stdout.readline` because it will
...
def search_pipeline_files(file_filters=None, max_files=30000):
    ...
    # TODO(stephen): Expose this as an argument and add as a separate command line flag
