...
def create_ids(cid_str):
    ...
    indicators = []
    while less_than(start, end, inclusive=True) and ALPHABET_LOCATION[start.letter] < 25:
...
def main():
    Flags.PARSER.add_argument(
        '--input_file', type=str, required=True, help='Raw csv lookup of cause of death to id.'
    )
    Flags.PARSER.add_argument(
        '--output_indicator_groups', type=str, required=True, help='File path for the cause of death indicators'
    )
    Flags.PARSER.add_argument(
        '--cause_of_death_lookup', type=str, required=True, help='File path for the cause of death lookup'
    )
    Flags.InitArgs()
    ...
    cause_of_death_indicators = [{'id': f"sim_cause_of_death_{g['slug']}", 'text': g['name'].strip()} for g in groups]
    cause_of_death_group = {
        'groupId': 'sim_cause_of_death',
        'groupText': 'Causa da Morte',
        'indicators': cause_of_death_indicators,
    }
    json_data = json.dumps([cause_of_death_group], indent=4)
    with open(Flags.ARGS.output_indicator_groups, 'w') as output_json:
        output_json.write(json_data)
    lookup = [{'cid_id': cid_id, 'field_id': f"sim_cause_of_death_{g['slug']}"} for g in groups for cid_id in g['indicators']]
