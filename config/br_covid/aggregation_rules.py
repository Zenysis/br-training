...
from config.br_covid.calculated_indicators import (
    CALCULATED_INDICATOR_CONSTITUENTS,
    CALCULATED_INDICATOR_FORMULAS,
)
from config.br_covid.indicators import GROUP_DEFINITIONS
...
def get_granularity_for_interval(
    ...
    # The epi week in Brazil follows the CDC definition which means it starts on Sunday.
    # Druid by default will group weeks starting on Monday, so we need to customize it.
    # TODO(stephen): This is the first deployment that does *not* use the WHO
    # definition. Build a standard implementation of this code for others to use.
    if requested_granularity == 'epi_week':
        closest_sunday = query_start_date - timedelta(
            days=query_start_date.weekday() + 1
        )
        return {
            'origin': closest_sunday.strftime(STANDARD_DATA_DATE_FORMAT),
            'period': 'P1W',
            'type': 'period',
        }
